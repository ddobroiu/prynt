  ...
  return (
    <div className="relative inline-block text-left">
      ...
      <select
        ...
        className={`appearance-none cursor-pointer pl-2.5 sm:pl-3 pr-7 sm:pr-8 py-1.5 rounded-lg text-xs font-bold border shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition-all touch-manipulation ${getStatusColor(status)}`}
        disabled={isLoading}
      >
      ...
      <div className="absolute right-1.5 sm:right-2 top-1/2 -translate-y-1/2 pointer-events-none text-current opacity-60">
...
export default function OrdersDashboard({ initialOrders = [] }: OrdersDashboardProps) {
  ...
  return (
    <div className="space-y-4 sm:space-y-6 md:space-y-8 p-3 sm:p-4 md:p-8 bg-gray-50/50 min-h-screen font-sans pb-safe">
      {/* HEADER */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 sm:p-5 md:p-6 rounded-xl sm:rounded-2xl shadow-sm border border-gray-100">
        <div className="flex items-center gap-3 sm:gap-4 w-full md:w-auto">
          <div className="flex-1">
            <h1 className="text-xl sm:text-2xl md:text-3xl font-extrabold text-gray-900 tracking-tight">Comenzi</h1>
            <p className="text-gray-500 mt-1 text-sm sm:text-base">Gestionează fluxul de producție.</p>
          </div>
          <button 
            onClick={() => router.refresh()} 
            className="min-w-10 min-h-10 sm:min-w-11 sm:min-h-11 flex items-center justify-center rounded-xl sm:rounded-full hover:bg-gray-100 active:bg-gray-200 text-gray-500 transition-colors touch-manipulation" 
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <div className="relative group flex-1 sm:flex-initial">
            ...
            <input
              ...
              className="pl-10 pr-4 min-h-11 sm:min-h-10 md:py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none w-full sm:w-72 transition-all bg-gray-50 focus:bg-white touch-manipulation"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
          </div>
          <select
            className="px-4 min-h-11 sm:min-h-10 md:py-2.5 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none bg-gray-50 focus:bg-white cursor-pointer transition-all touch-manipulation"
            value={statusFilter}
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-5 md:gap-6">
        {filteredOrders.length === 0 ? (
          <div className="text-center py-12 sm:py-16 bg-white rounded-xl sm:rounded-2xl border border-gray-200 border-dashed">
            ...
            <h3 className="text-base sm:text-lg font-semibold text-gray-900">Nicio comandă găsită</h3>
          </div>
        ) : (
          filteredOrders.map((order) => {
            ...
            return (
              ...
              >
                {/* CARD PRINCIPAL */}
                <div className="p-4 sm:p-5 md:p-6 grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-5 md:gap-6 items-start relative">
                  
                  {/* STATUS */}
                  <div className="lg:col-span-3 flex flex-col gap-2">
                    <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                      <div className="bg-indigo-50 text-indigo-700 px-2.5 sm:px-3 py-1 rounded-lg text-base sm:text-lg font-bold font-mono border border-indigo-100">
                        #{order.orderNo}
                      </div>
                  ...
                  <div className="lg:col-span-4">
                    <div className="bg-gray-50 rounded-lg sm:rounded-xl p-3 sm:p-4 border border-gray-100 h-full">
                      <div className="flex items-start gap-2.5 sm:gap-3">
                        <div className="mt-0.5 bg-white p-1.5 sm:p-2 rounded-lg shadow-sm text-gray-600 border border-gray-200 shrink-0">
                          <User className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                        </div>
                        <div className="text-xs sm:text-sm min-w-0 flex-1">
                          <p className="font-bold text-gray-900 truncate">{displayName}</p>
                          <div className="flex items-center gap-2 text-gray-600 mt-0.5">
                            <Mail className="w-3 h-3 shrink-0" /> <span className="truncate">{displayEmail}</span>
                          </div>
                      ...
                      <div className="mt-3 pt-3 border-t border-gray-200 flex items-start gap-2.5 sm:gap-3">
                        <div className="mt-0.5 text-indigo-600 shrink-0">
                          <MapPin className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                        </div>
                        <div className="text-xs sm:text-sm text-gray-700 leading-snug w-full min-w-0">
                          <div className="truncate">{address?.localitate || "Localitate lipsă"}, {address?.judet || ""} {address?.postCode ? `(${address.postCode})` : ""}</div>
                  ...
                  <div className="lg:col-span-3">
                    <div className="flex flex-col h-full justify-center p-3 sm:p-4 rounded-lg sm:rounded-xl bg-white border border-gray-100 shadow-sm">
                        <div className="flex items-center gap-2.5 sm:gap-3 mb-2.5 sm:mb-3">
                          <div className={`p-1.5 sm:p-2 rounded-lg shrink-0 ${isCard ? 'bg-emerald-100 text-emerald-600' : 'bg-orange-100 text-orange-600'}`}>
                            {isCard ? <CreditCard className="w-4 h-4 sm:w-5 sm:h-5" /> : <Banknote className="w-4 h-4 sm:w-5 sm:h-5" />}
                          </div>
                      ...
                      <div className="flex justify-between items-end border-t border-gray-100 pt-2.5 sm:pt-3">
                          <span className="text-xs sm:text-sm text-gray-500">Total</span>
                          <span className="text-lg sm:text-xl font-black text-gray-900">{formatMoney(order.total)}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="lg:col-span-2 flex justify-end items-center h-full">
                    <button 
                        ...
                        className={`flex items-center justify-center gap-2 w-full lg:w-auto px-4 min-h-11 sm:min-h-10 md:py-3 rounded-xl font-semibold text-sm transition-all touch-manipulation ${
                          isExpanded 
                            ? "bg-indigo-600 text-white shadow-md shadow-indigo-200 active:bg-indigo-700" 
                    ...
                    <div className="border-t border-gray-100 bg-gray-50/30 p-6 animate-in slide-in-from-top-2 duration-200">
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                    ...
                                    {order.items.map((item) => (
                                        <div key={item.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                                            <div className="flex items-center gap-4 w-full sm:w-auto">
                                                <div className="w-12 h-12 bg-gray-100 rounded-lg shrink-0 flex items-center justify-center text-gray-400 border border-gray-200">
                                                    ...
                                                    {item.metadata && (
                                                      <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 text-xs text-gray-600 bg-gray-50/50 p-2 rounded border border-gray-100">
                                                          {Object.entries(item.metadata).map(([key, value]) => {
                                                            ...
                                                            return (
                                                              <div key={key} className="flex gap-1 wrap-break-word">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-5 md:gap-6">
                                    ...
                                    <div className="bg-white p-3 sm:p-4 rounded-lg sm:rounded-xl border border-gray-200 shadow-sm">
                                        <h4 className="flex items-center gap-2 text-xs font-bold text-gray-500 uppercase tracking-wider mb-3 border-b pb-2">
                                            <Building2 className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Date Facturare
                                        </h4>
                                    ...
                                    <div className="bg-white p-3 sm:p-4 rounded-lg sm:rounded-xl border border-gray-200 shadow-sm">
                                        <h4 className="flex items-center gap-2 text-xs font-bold text-gray-500 uppercase tracking-wider mb-3 border-b pb-2">
                                            <Megaphone className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Marketing & Sursă
                                        </h4>
                                        {order.marketing ? (
                            ...
                            <div className="lg:col-span-1">
                                <div className="bg-white p-4 sm:p-5 rounded-lg sm:rounded-xl border border-gray-200 shadow-sm h-full lg:sticky lg:top-6">
                                    <h4 className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3 sm:mb-4 border-b border-gray-100 pb-2">Acțiuni Rapide</h4>
                                    <div className="space-y-3 sm:space-y-4">
                                        <Link href={`/admin/orders/${order.id}`} className="flex items-center justify-center w-full gap-2 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white px-4 min-h-11 sm:min-h-10 sm:py-2.5 rounded-lg font-semibold transition-all shadow-sm touch-manipulation">
                                        ...
                                        {order.awbNumber ? (
                                            <div className="flex flex-col gap-2 p-3 rounded-lg bg-indigo-50 border border-indigo-100">
                                                <div className="flex items-center justify-between gap-2">
                                                    <div className="flex items-center gap-2 text-indigo-700 font-bold text-xs min-w-0 flex-1">
                                                        <Truck className="w-4 h-4 shrink-0" />
                                                        AWB: <span className="font-mono select-all truncate">{order.awbNumber}</span>
                                                    </div>
                                                ...
                                                <a 
                                                    ...
                                                    target="_blank" 
                                                    rel="noreferrer"
                                                    className="w-full flex items-center justify-center gap-2 bg-white border border-indigo-200 text-indigo-700 hover:bg-indigo-50 active:bg-indigo-100 px-3 min-h-9 rounded text-xs font-bold transition-colors touch-manipulation"
                                                >
                                            ...
                                            <button 
                                                ...
                                                className="flex flex-col items-center justify-center gap-2 bg-gray-50 hover:bg-gray-100 active:bg-gray-200 border border-gray-200 text-gray-700 p-3 min-h-20 rounded-lg text-xs font-bold transition-colors disabled:opacity-60 touch-manipulation"
                                            >
