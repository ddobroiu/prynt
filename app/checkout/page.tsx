...
import { User, Mail, Phone, Home, Building, Loader2, MapPin, CheckSquare, Square } from 'lucide-react';
...
function CheckoutForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (billing.tip_factura === 'companie') {
            finalBillingPayload.cui = billing.cui;
        } else {
             finalBillingPayload.name = address.nume_prenume;
             finalBillingPayload.address = sameAsDelivery ? `${address.judet}, ${address.localitate}, ${address.strada_nr}` : billing.address;
        }

        try {
            const response = await fetch('/api/order/create', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ address, billing: finalBillingPayload, cart }) });
            const result = await response.json();
            if (!response.ok) throw new Error(result.message || 'A apărut o eroare.');
            setFormState('success');
            alert(`Comandă plasată! Factura: ${result.invoiceLink}. AWB DPD: ${result.awbNumber}`);
        } catch (error: any) {
            setFormState('error');
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-10">
            ...
            <div className="bg-gray-900 border border-gray-700 rounded-2xl p-8"><h2 className="text-2xl font-bold text-white mb-6">1. Date Livrare</h2><div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8">
                ...
                <FormInput name="localitate" label="Localitate (manual)" icon={<MapPin />} state={address} setState={setAddress} />
                <FormInput name="strada_nr" label="Stradă, Număr, Bloc, etc." icon={<Home />} state={address} setState={setAddress} colSpan={2} />
            </div></div>

            {/* --- SECȚIUNEA DATE DE FACTURARE --- */}
            ...
            <button type="submit" disabled={formState === 'loading'} className="w-full bg-green-600 text-white font-bold py-4 text-xl rounded-lg hover:bg-green-700 transition-all duration-300 transform hover:scale-[1.02] disabled:bg-gray-500 disabled:cursor-wait flex items-center justify-center">{formState === 'loading' ? <><Loader2 className="animate-spin mr-3"/> Se procesează...</> : 'Plasează Comanda'}</button>
            {formState === 'error' && <p className="text-red-400 text-center mt-4">{errorMessage}</p>}
        </form>
    );
}
