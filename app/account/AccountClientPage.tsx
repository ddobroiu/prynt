...
import UserDetailsForm from "@/components/UserDetailsForm"; // <-- NOU
...
// NOU: Helper pentru a afișa sumarul produselor comandate
const formatOrderItems = (items: any[]) => {
  if (!items || items.length === 0) return "Fără produse";
  // Extrage primele 2 produse și le concatenează
  const summary = items.slice(0, 2).map((item: any) => `${item.qty}x ${item.name}`).join(', ');
  if (items.length > 2) {
      return `${summary} și încă ${items.length - 2} produse.`;
  }
  return summary;
};
...
export default function AccountClientPage({ orders = [], billing }: AccountClientPageProps) {
  ...
  // Tab nou 'details' adăugat în listă
  const [tab, setTab] = useState<'orders' | 'addresses' | 'security' | 'details'>('orders');
  ...
  useEffect(() => {
    ...
    else if (tabParam === 'details') setTab('details'); // <-- NOU
  }, [searchParams]);

  const handleTabChange = (newTab: 'orders' | 'addresses' | 'security' | 'details') => {
    setTab(newTab);
  ...
  return (
    <div className="min-h-[60vh] p-4 md:p-8 bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <aside className="w-full lg:w-64 shrink-0 space-y-2">
            <button onClick={() => handleTabChange('orders')} className={`w-full text-left px-4 py-3 rounded-lg font-medium transition-colors ${tab === 'orders' ? 'bg-indigo-600 text-white shadow-md' : 'bg-white dark:bg-gray-800 text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700'}`}>Comenzile mele</button>
            <button onClick={() => handleTabChange('addresses')} className={`w-full text-left px-4 py-3 rounded-lg font-medium transition-colors ${tab === 'addresses' ? 'bg-indigo-600 text-white shadow-md' : 'bg-white dark:bg-gray-800 text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700'}`}>Adrese de livrare</button>
            <button onClick={() => handleTabChange('details')} className={`w-full text-left px-4 py-3 rounded-lg font-medium transition-colors ${tab === 'details' ? 'bg-indigo-600 text-white shadow-md' : 'bg-white dark:bg-gray-800 text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700'}`}>Detaliile contului</button> {/* <-- NOU */}
            <button onClick={() => handleTabChange('security')} className={`w-full text-left px-4 py-3 rounded-lg font-medium transition-colors ${tab === 'security' ? 'bg-indigo-600 text-white shadow-md' : 'bg-white dark:bg-gray-800 text-gray-900 dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700'}`}>Securitate</button>
          </aside>
          ...
          <main className="flex-1 bg-white dark:bg-gray-800 rounded-xl p-6 shadow-sm border border-gray-100 dark:border-gray-700">
            {tab === 'orders' && (
               <div className="animate-fadeIn">
                 <h2 className="text-xl font-semibold mb-6 pb-4 border-b dark:border-gray-700 text-gray-900 dark:text-white">Istoric comenzi</h2>
                 ...
                 {orders.length === 0 ? (
                   ...
                   <ul className="space-y-4">
                     {orders.map((o: any) => {
                       ...
                       const orderItemsSummary = formatOrderItems(o.items); // Utilizarea noului helper
                         ...
                         <li key={o.id} className="p-5 rounded-xl border border-gray-200 dark:border-gray-700 bg-slate-50 dark:bg-slate-800/40 hover:bg-slate-100 dark:hover:bg-slate-800 transition">
                           <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                             ...
                             <div className="text-right shrink-0">
                                 <div className="text-xl font-extrabold text-gray-900 dark:text-white">{new Intl.NumberFormat("ro-RO", { style: "currency", currency: "RON" }).format(o.total)}</div>
            ...
            {tab === 'details' && <div className="animate-fadeIn"><UserDetailsForm session={session} /></div>} {/* <-- NOU TAB RENDER */}
            {tab === 'security' && (
              <div className="animate-fadeIn space-y-8">
                <h2 className="text-xl font-semibold mb-6 pb-4 border-b dark:border-gray-700 text-gray-900 dark:text-white">Securitate și parolă</h2>
                <ChangePasswordForm />
              </div>
            )}
          </main>
