...
export default async function OrdersPage() {
  ...
  const orders = await prisma.order.findMany({
    ...
    select: { id: true, orderNo: true, createdAt: true, total: true, paymentType: true },
  });
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-16">
      ...
      {orders.length === 0 ? (
        <div className="rounded-md border border-[--border] p-4 text-muted">Nu ai comenzi încă.</div>
      ) : (
        <ul className="space-y-3">
          {orders.map((o) => (
            <li key={o.id} className="rounded-md border border-[--border] p-4 flex items-center justify-between">
              <div>
              ...
              <div className="text-right">
                <div className="font-semibold">{fmtRON(Number(o.total))}</div>
                <div className="text-xs text-muted">Plată: {o.paymentType}</div>
              </div>
              <div className="ml-4">
                <Link href={`/account/orders/${o.id}`} className="rounded-md border border-[--border] px-3 py-1.5 text-sm hover:bg-white/5">Detalii</Link>
              </div>
            </li>
          ))}
        </ul>
