...
export async function generateStaticParams() {
  ...
  const filtered = slugs.filter((s) => {
    ...
    return !!p && p.metadata?.category === "pliante";
  });
...
export default async function Page({ params }: Props) {
  ...
  if (!product || product.metadata?.category !== "pliante") {
    return notFound();
  }
      ...
      <section style={{ marginTop: 18 }}>
        <header style={{ marginBottom: 18 }}>
          <h1 style={{ fontSize: 28, fontWeight: 700, textAlign: "center", margin: 0 }}>
            {product.title || `Pliante ${joinedSlug}`}
          </h1>
          <p style={{ marginTop: 8, color: "#9ca3af", textAlign: "center", marginBottom: 0 }}>{product.description}</p>
        </header>

        <PlianteConfigurator productSlug={product.slug} initialWidth={initialWidth ?? undefined} initialHeight={initialHeight ?? undefined} />
      </section>
    </main>
  );
}